...
const { defineParameterType, supportCodeLibraryBuilder } = require("cucumber");
const {
  CucumberExpression,
  RegularExpression,
...
supportCodeLibraryBuilder.parameterTypeRegistry =
  stepDefinitionRegistry.options.parameterTypeRegistry;
...
function resolveDataTable(argument, exampleRowData, replaceParameterTags) {
  if (!exampleRowData) {
...
function resolveDocString(argument) {
...
module.exports = {
  ...
  resolveAndRunStepDefinition(
    ...
    if (expression && implementation) {
      ...
      if (step.dataTable) {
        argument = resolveDataTable(
          step.dataTable,
          exampleRowData,
          replaceParameterTags
        );
      } else if (step.docString) {
        argument = resolveDocString(step.docString);
      }
